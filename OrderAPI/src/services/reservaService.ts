...
export class ReservaService {
  ...
  async reserveAuto(quantidadePessoas: number, data: string, hora: string, nomeCliente?: string, telefone?: string) {
    return this.reserveFlexible(quantidadePessoas, data, hora, nomeCliente, telefone);
  }

  async reserveFlexible(quantidadePessoas: number, data: string, hora: string, nomeCliente?: string, telefone?: string) {
    await this.cleanupExpired();
    const settings = await this.settingsService.getSettings();
    ...
    for (let inicio = 1; inicio <= settings.totalMesas - tablesNeeded + 1; inicio++) {
      const mesasNecessarias = Array.from({ length: tablesNeeded }, (_, i) => inicio + i);
      const mesasIndisponiveis = mesasNecessarias.filter(
        (m) => mesasComConflito.has(m) || bloqueadas.has(m)
      );

      if (mesasIndisponiveis.length > 0) continue;
      ...
      if (reservasCriadas.length === tablesNeeded) {
        return {
